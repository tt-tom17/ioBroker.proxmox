...
class ProxmoxUtils {
    ...
    async getNodeVersion(node, useCache) {
        return new Promise((resolve, reject) => {
            if (useCache && this.responseCache[`/nodes/${node}/version`]) {
                resolve(JSON.parse(JSON.stringify(this.responseCache[`/nodes/${node}/version`])));
            } else {
                this._getData(`/nodes/${node}/version`, 'get', '', '', 'getNodeVersion')
                    .then((data) => {
                        if (data !== '' && data !== null && typeof data === 'object') {
                            this.responseCache[`/nodes/${node}/version`] = data.data;
                            resolve(data.data);
                        } else {
                            reject();
                        }
    ...
    async getBackupStatus(node, storage) {
        return new Promise((resolve, reject) => {
            this.adapter.log.debug(`[getBackupStatus] Node: ${node} Storage: ${storage}`);
    ...
    async getStorageStatus(node, ID, shared, useCache) {
        return new Promise((resolve, reject) => {
            if (useCache && this.responseCache[`/nodes/${shared ? 'SHARED' : node}/storage/${ID}/status`]) {
                ...
                this.adapter.log.debug(`[getStorageStatus] using SHARED ${shared}`);
